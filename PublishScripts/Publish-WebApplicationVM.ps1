...
<#
.SYNOPSIS
Visual Studio Web プロジェクトに Microsoft Azure 仮想マシンを作成して配置します。
...
function Publish-AzureWebApplicationToVM
{
    ...
    do 
    {
        ...
        if ($result)
        {
            Write-VerboseWithTime ($scriptName + ' VM に公開できました。')
        }
        ...
        else
        {
         ...
         #新しく作成し、Web Deploy をインストールしている仮想マシンにのみ再公開してください。 
    ...
    if (!$result)
    {                    
        Write-Warning ' 仮想マシンへの公開に失敗しました。信頼されていない証明書または無効な証明書が原因と考えられます。-AllowUntrusted を指定して、信頼されていない証明書を受け入れることができます。'
        throw ($scriptName + ' VM に公開できませんでした。')
    }
}
...
try
{
    ...
    {
         throw 'Azure PowerShell を読み込むことができません。最新バージョンをインストールするには、http://go.microsoft.com/fwlink/?LinkID=320552 に移動してください。既に Azure PowerShell をインストールしている場合は、コンピューターを再起動するか、手動でモジュールをインポートする必要があることがあります。'
    }
