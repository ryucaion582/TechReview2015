#  AzureVMPublishModule.psm1 は Windows PowerShell スクリプト モジュールです。このモジュールは、Web アプリケーションのライフ サイクル管理を自動化する Windows PowerShell 関数をエクスポートします。この関数をそのまま使用するか、アプリケーションや公開環境用にカスタマイズすることができます。
...
<#
.SYNOPSIS
現在のサブスクリプションで "devtest*" という名前の Microsoft Azure ストレージ アカウントを見つけます。
...
.PARAMETER  Location
ストレージ アカウントの場所を指定します。有効な値は、"West US" などの Microsoft Azure の場所です。場所またはアフィニティ グループを入力できますが、両方を入力することはできません。
...
<#
.SYNOPSIS
名前が "devtest" で始まる新しい Microsoft Azure ストレージ アカウントを作成します。

.DESCRIPTION
Add-AzureVMStorage 関数は、現在のサブスクリプションに新しい Microsoft Azure ストレージ アカウントを作成します。アカウントの名前は "devtest" で始まり、その直後には一意の英数字文字列が使用されます。関数は新しいストレージ アカウントの名前を返します。新しいストレージ アカウントの場所またはアフィニティ グループを指定する必要があります。
...
function Add-AzureVMStorage
{
    ...
    # 場所/アフィニティ グループを使用して新しい Microsoft Azure ストレージ アカウントを作成します
...
<#
.SYNOPSIS
Microsoft Azure サブスクリプション内の新しい仮想マシンの全要素を作成します。

.DESCRIPTION
この関数は、Microsoft Azure 仮想マシン (VM) を作成し、配置された VM の URL を返します。関数は前提条件を設定してから、New-AzureVM コマンドレット (Azure モジュール) を呼び出して新しい VM を作成します。 
...
-- Add-AzureVM コマンドレットを呼び出して、新しい Microsoft Azure 仮想マシンを作成し、新しい VM を返します。通常、関数パラメーターの値は、Microsoft Azure に統合された Web プロジェクト用に Visual Studio によって生成される JSON 構成ファイルから取得しています。このモジュールの Read-ConfigFile 関数は、JSON ファイルをハッシュ テーブルに変換します。ハッシュ テーブルの cloudservice キーを (PSCustomObject として) 変数に保存し、カスタム オブジェクトのプロパティをパラメーター値として使用します。
...
既存の Microsoft Azure サービス、または新しい Microsoft Azure サービスの名前を指定します。この値は、New-AzureVM コマンドレットの ServiceName パラメーターに送信されます。このコマンドレットは、新しい仮想マシンを既存の Microsoft Azure サービスに追加するか、Location または AffinityGroup が指定されている場合は現在のサブスクリプションに新しい仮想マシンとサービスを作成します。このパラメーターは必須です。 
...
新しい VM の可用性セットの名前を指定します。複数の仮想マシンを単一の可用性セットに配置すると、Microsoft Azure は、いずれかの仮想マシンで障害が発生した場合のサービス継続性を向上するために、各仮想マシンを異なるホスト上に配置しようとします。このパラメーターは省略可能です。 

.PARAMETER  VNetName
新しい仮想マシンを配置する仮想ネットワーク名の名前を指定します。この値は、Add-AzureVM コマンドレットの VNetName パラメーターに送信されます。このパラメーターは省略可能です。 

.PARAMETER  Location
新しい VM の場所を指定します。有効な値は、"West US" などの Microsoft Azure の場所です。既定値はサブスクリプションの場所です。このパラメーターは省略可能です。 

.PARAMETER  AffinityGroup
新しい VM のアフィニティ グループを指定します。アフィニティ グループは、関連するリソースのグループです。アフィニティ グループを指定すると、Microsoft Azure は、効率を向上するためにリソースをグループにまとめようとします。 
...
.PARAMETER VMImage
ImageName が OSImage ではなく VMImage の名前であると指定します。このパラメーターは省略可能です。このパラメーターを指定しない場合、ImageName は OSImage として扱われます。このパラメーターの値は、Visual Studio によって仮想マシン向けに生成される JSON 構成ファイルに含まれています。

.PARAMETER GeneralizedImage
VMImage について、OS の状態が汎用かどうかを指定します。このパラメーターは省略可能です。このパラメーターを指定しない場合、スクリプトは特殊な VMImage を扱うときと同様に動作します。このパラメーターは OSImages では無視されます。このパラメーターの値は、Visual Studio によって仮想マシン向けに生成される JSON 構成ファイルに含まれています。
...
function Add-AzureVM
{
    ...
    # 新しい Microsoft Azure VM 構成オブジェクトを作成します。
    ...
    # 現時点では、VMImages は場所をサポートしていません。新しい VM はイメージが置かれている同じストレージ アカウント (場所) に作成されます。
...
<#
.SYNOPSIS
指定された Microsoft Azure 仮想マシンを取得します。

.DESCRIPTION
Find-AzureVM 関数は、Microsoft Azure 仮想マシン (VM) をサービス名と VM 名に基づいて取得します。この関数は Test-AzureName コマンドレット (Azure モジュール) を呼び出して、サービス名が Microsoft Azure に存在することを検証します。存在する場合は、Get-AzureVM コマンドレットを呼び出して VM を取得します。この関数は、vm キーと foundService キーを含むハッシュ テーブルを返します。
...
.PARAMETER  ServiceName
既存の Microsoft Azure サービスの名前。このパラメーターは必須です。
...
<#
.SYNOPSIS
Web パッケージを Microsoft Azure に配置します。

.DESCRIPTION
Publish-WebPackage 関数は、MsDeploy.exe と Web 配置パッケージ ZIP ファイルを使用して、リソースを Microsoft Azure Web サイトに配置します。この関数は出力を生成しません。MSDeploy.exe の呼び出しが失敗した場合、関数は例外をスローします。詳細な出力を取得するには、Verbose 共通パラメーターを使用します。
...
.PARAMETER Password
Web サイト管理者のパスワードを指定します。パスワードはプレーン テキスト形式で入力します。セキュリティで保護された文字列は許可されていません。このパラメーターは必須です。
